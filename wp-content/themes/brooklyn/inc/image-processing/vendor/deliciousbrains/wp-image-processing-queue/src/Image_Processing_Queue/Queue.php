...
class Queue {
	...
	protected function process_image( $post_id, $sizes ) {
		...
		foreach ( $sizes as $size ) {
			...
			if ( self::is_attachment_locked( $post_id, $size ) ) {
				return;
			}
			...
			self::lock_attachment( $post_id, $size );
	...
	public static function get_size_name( $size ) {
		...
		$upscale = $size[3] ? 'true' : 'false';

		return 'ipq-' . $size[0] . 'x' . $size[1] . '-' . $crop . '-' . $upscale;
	...
	public static function is_size_larger_than_original( $post_id, $size ) {
		...
		if( ! $size[3]  && ( $size[0] > $image_meta['width'] || $size[1] > $image_meta['height'] ) ) {
			return true;
		}
	 ...
	 * @return bool
	 */
	public static function is_attachment_locked( $post_id, $size = array() ) {

		$image_meta = self::get_image_meta( $post_id );

		if ( isset( $image_meta['ipq_locked'] ) && is_array( $image_meta['ipq_locked'] ) && in_array( self::lock_string( $size ), $image_meta['ipq_locked'] ) ) {
			return true;
		}
	 ...
	 * @param array $size
	 */
	public static function lock_attachment( $post_id, $size = array() ) {
		...
		$image_meta['ipq_locked'][]    = self::lock_string( $size );
		$image_meta['ipq_locked_time'] = date('Y-m-d G:i:s');

		wp_update_attachment_metadata( $post_id, $image_meta );

	}

	public static function lock_string( $size = array() ) {
