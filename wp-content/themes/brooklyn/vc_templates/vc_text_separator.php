...
if( 'custom' === $color && '' !== $accent_color ) {
	...
	if( function_exists( 'ut_is_gradient' ) && ut_is_gradient( $accent_color ) ) {

		$css_style = "#{$id} .vc_sep_holder {";

		$css_style .= "background-image: -webkit-$accent_color;";
		$css_style .= "background-image: -o-$accent_color;";
		$css_style .= "background-image: $accent_color;";
		$css_style .= "border-top-width: 0;";
		$css_style .= "height: {$border_width}px;";

		$css_style .= '}';

		$css_style .= "#{$id} .vc_sep_holder .vc_sep_line { border-color: transparent; }";

		if( !empty( $css_style ) ) {

			$css_style = '<style type="text/css">' . $css_style . '</style>';

		}

	} else {

		$inline_css = ' style="' . vc_get_css_color( 'border-color', $accent_color ) . '"';
...
$css_style
<div id="$id" class="$css_class" $wrapper_attributes_html><span class="vc_sep_holder vc_sep_holder_l"><span $inline_css class="vc_sep_line"></span></span>$content<span class="vc_sep_holder vc_sep_holder_r"><span $inline_css class="vc_sep_line"></span></span>
</div>
TEMPLATE;
echo $separatorHtml;
