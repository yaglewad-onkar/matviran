...
class Helpers {
	 ...
	 */
	public static function download_import_files( $import_file_info, $type = 'wpbakery' ) {
		$downloaded_files = array();
		$downloader = new Downloader();

		// CUSTOM CODE
		$purchase_code = get_option( 'ohio_license_code' );
		if ( !empty( $import_file_info['import_file_url'] ) ) {
			$import_file_info['import_file_url'] .= '/' . $purchase_code;
		}
		if ( !empty( $import_file_info['import_elementor_file_url'] ) ) {
			$import_file_info['import_elementor_file_url'] .= '/' . $purchase_code;
		}
		if ( !empty( $import_file_info['import_widget_file_url'] ) ) {
			$import_file_info['import_widget_file_url'] .= '/' . $purchase_code;
		}
		if ( !empty( $import_file_info['import_options_file_url'] ) ) {
			$import_file_info['import_options_file_url'] .= '/' . $purchase_code;
		}
		if ( !empty( $import_file_info['import_sliders_file_url'] ) ) {
			$import_file_info['import_sliders_file_url'] .= '/' . $purchase_code;
		}
		// / CUSTOM CODE
		...
		if ( empty( $import_file_info['import_file_url'] ) ) {
			if ( file_exists( $import_file_info['local_import_file'] ) ) {
				...
				return new \WP_Error(
					...
					sprintf(
						__( '"import_file_url" or "local_import_file" for %s%s%s are not defined!', 'ohio-importer' ),
						'<strong>',
			...
			if ( $type == 'elementor' ) {
				$downloaded_files['content'] = $downloader->download_file( $import_file_info['import_elementor_file_url'], $content_filename );
			} else {
		...
		// ----- Set options file path -----
		// Get options file as well. If defined!
		if ( ! empty( $import_file_info['import_options_file_url'] ) ) {
			// Set the filename string for widgets import file.
			$options_filename = apply_filters( 'pt-ocdi/downloaded_options_file_prefix', 'demo-options-import-file_' ) . self::$demo_import_start_time . apply_filters( 'pt-ocdi/downloaded_options_file_suffix_and_file_extension', '.json' );

			// Download the widgets import file.
			$downloaded_files['options'] = $downloader->download_file( $import_file_info['import_options_file_url'], $options_filename );
		...
		// ----- Set sliders file path -----
		// Get sliders zip file as well. If defined!
		if ( ! empty( $import_file_info['import_sliders_file_url'] ) ) {
			// Set the filename string for widgets import file.
			$sliders_filename = apply_filters( 'pt-ocdi/downloaded_sliders_file_prefix', 'demo-sliders-import-file_' ) . self::$demo_import_start_time . apply_filters( 'pt-ocdi/downloaded_sliders_file_suffix_and_file_extension', '.zip' );

			// Download the widgets import file.
			$downloaded_files['sliders'] = $downloader->download_file( $import_file_info['import_sliders_file_url'], $sliders_filename );

			// Return from this function if there was an error.
			if ( is_wp_error( $downloaded_files['sliders'] ) ) {
				return $downloaded_files['sliders'];
			}
		}
	...
	public static function write_to_file( $content, $file_path ) {
		...
		if ( ! $wp_filesystem->put_contents( $file_path, $content ) ) {
			return new \WP_Error(
				...
				sprintf(
					__( 'An error occurred while writing file to your server! Tried to write a file to: %s%s.', 'ohio-importer' ),
					'<br>',
					$file_path
				)
			);
	...
	public static function data_from_file( $file_path ) {
		...
		if ( ! $data ) {
			return new \WP_Error(
				...
				sprintf(
					__( 'An error occurred while reading a file from your server! Tried reading file from path: %s%s.', 'ohio-importer' ),
					'<br>',
					$file_path
				)
			);
	...
	private static function check_wp_filesystem_credentials() {
		...
		if ( ! ( 'direct' === get_filesystem_method() ) ) {
			return new \WP_Error(
				...
				sprintf(
					__( 'This WordPress page does not have %sdirect%s write file access. This plugin needs it in order to save the demo import xml file to the upload directory of your site. You can change this setting with these instructions: %s.', 'ohio-importer' ),
					'<strong>',
					'</strong>',
		...
		$plugin_page_setup = apply_filters( 'pt-ocdi/plugin_page_setup', array(
				...
				'page_title'  => esc_html__( 'One Click Demo Import' , 'ohio-importer' ),
				'menu_title'  => esc_html__( 'Import Demo Data' , 'ohio-importer' ),
				'capability'  => 'import',
		...
		if ( false === ( $creds = request_filesystem_credentials( $demo_import_page_url, '', false, false, null ) ) ) {
			return new \WP_error(
				...
				__( 'An error occurred while retrieving reading/writing permissions to your server (could not retrieve WP filesystem credentials)!', 'ohio-importer' )
			);
		}
		...
		if ( ! WP_Filesystem( $creds ) ) {
			return new \WP_Error(
				...
				__( 'Your WordPress login credentials don\'t allow to use WP_Filesystem!', 'ohio-importer' )
			);
		}
	...
	public static function register_file_as_media_attachment( $log_path ) {
		...
		$attachment = array(
			...
			'post_title'     => apply_filters( 'pt-ocdi/attachment_prefix', esc_html__( 'One Click Demo Import - ', 'ohio-importer' ) ) . preg_replace( '/\.[^.]+$/', '', basename( $log_path ) ),
			'post_content'   => '',
			'post_status'    => 'inherit',
		);
	...
	public static function verify_ajax_call() {
		...
		if ( ! current_user_can( 'import' ) ) {
			wp_die(
				sprintf(
					__( '%sYour user role isn\'t high enough. You don\'t have permission to import demo data.%s', 'ohio-importer' ),
	...
	public static function process_uploaded_files( $uploaded_files, $log_file_path ) {
		...
		if ( empty( $content_file_info['file'] ) || isset( $content_file_info['error'] ) ) {
			...
			self::log_error_and_send_ajax_response(
				__( 'Please upload XML file for content import. If you want to import widgets or customizer settings only, please use Widget Importer & Exporter or the Customizer Export/Import plugin.', 'ohio-importer' ),
				$log_file_path,
				esc_html__( 'Upload files', 'ohio-importer' )
			);
		}
		...
		if ( $widget_file_info && ! isset( $widget_file_info['error'] ) ) {
			...
			$log_added = self::append_to_file(
				sprintf(
					__( 'Widget file was not uploaded. Error: %s', 'ohio-importer' ),
		...
		if ( $customizer_file_info && ! isset( $customizer_file_info['error'] ) ) {
			...
			$log_added = self::append_to_file(
				sprintf(
					__( 'Customizer file was not uploaded. Error: %s', 'ohio-importer' ),
		...
		if ( $redux_file_info && ! isset( $redux_file_info['error'] ) ) {
			if ( isset( $_POST['redux_option_name'] ) && empty( $_POST['redux_option_name'] ) ) {
				// Write error to log file and send an AJAX response with the error.
				self::log_error_and_send_ajax_response(
					esc_html__( 'Missing Redux option name! Please also enter the Redux option name!', 'ohio-importer' ),
					$log_file_path,
					esc_html__( 'Upload files', 'ohio-importer' )
				);
			}
			...
			$log_added = self::append_to_file(
				sprintf(
					__( 'Redux file was not uploaded. Error: %s', 'ohio-importer' ),
		...
		$log_added = self::append_to_file(
			__( 'The import files were successfully uploaded!', 'ohio-importer' ) . self::import_file_info( $selected_import_files ),
			$log_file_path,
			esc_html__( 'Upload files' , 'ohio-importer' )
		);
	...
	public static function import_file_info( $selected_import_files ) {
		...
		return PHP_EOL .
		sprintf(
			__( 'Initial max execution time = %s', 'ohio-importer' ),
			ini_get( 'max_execution_time' )
		) . PHP_EOL .
		sprintf(
			__( 'Files info:%1$sSite URL = %2$s%1$sData file = %3$s%1$sWidget file = %4$s%1$sCustomizer file = %5$s%1$sRedux files:%1$s%6$s', 'ohio-importer' ),
			PHP_EOL,
			get_site_url(),
			$selected_import_files['content'],
			empty( $selected_import_files['widgets'] ) ? esc_html__( 'not defined!', 'ohio-importer' ) : $selected_import_files['widgets'],
			empty( $selected_import_files['customizer'] ) ? esc_html__( 'not defined!', 'ohio-importer' ) : $selected_import_files['customizer'],
			empty( $redux_file_string ) ? esc_html__( 'not defined!', 'ohio-importer' ) : $redux_file_string
		);
	}
